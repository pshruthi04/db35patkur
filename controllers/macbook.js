...
exports.macbook_list = async function(req, res) { 
    try{ 
        themacbooks = await macbook.find(); 
        res.send(themacbooks); 
    } 
    catch(err){ 
        res.status(500); 
        res.send(`{"error": ${err}}`); 
...
${JSON.stringify(req.body)}`) 
    try { 
        ...
        if(req.body.macbook_type)  toUpdate.cost = req.body.cost; 
...
exports.macbook_view_all_Page = async function(req, res) { 
    try{ 
        themacbooks = await macbook.find(); 
        res.render('macbook', { title: 'macbook Search Results', results: themacbooks }); 
    } 
    catch(err){ 
        res.status(500); 
        res.send(`{"error": ${err}}`); 
